...
export default function ProductDetails() {
  ...
  const [product, setProduct] = useState(undefined); // undefined = a carregar, null = não existe
  ...
  const [phase, setPhase] = useState('form'); // form | submitting | pending | delivered | failed
  const [error, setError] = useState('');
  const [order, setOrder] = useState(null);       // { order_id, payment_id, gateway }
  const [credential, setCredential] = useState('');
  const pollRef = useRef(null);

  const method = PAYMENT_METHODS.find(m => m.value === formData.pagamento) || PAYMENT_METHODS[0];

  // Sondagem do estado do pedido enquanto está pendente.
  useEffect(() => {
    ...
    async function tick() {
      try {
        ...
        if (r.status === 'delivered') {
          ...
          setPhase('failed');
          setError('O pagamento foi cancelado ou falhou. Tenta novamente.');
        }
  ...
  // Instruções de pagamento vindas do gateway (nomes de campo defensivos).
  ...
  return (
    <div className="container animate-fade-in">
      <Link to="/" className="flex items-center gap-2 text-secondary hover:text-accent-color mb-6" style={{ width: 'fit-content' }}>
      ...
      <div className="details-grid">
        ...
        <div>
          ...
          <div className="glass p-4 rounded-lg mb-6">
            ...
            <div className="info-item"><ShieldCheck className="info-icon" size={20} /><span>Entrega automática após pagamento confirmado</span></div>
          </div>
        </div>

        {/* Checkout */}
        <div>
          <div className="checkout-form mt-0">
            {phase === 'delivered' ? (
              <DeliveredPanel product={product} credential={credential} channels={order?.delivery_channels} />
            ) : phase === 'pending' ? (
              <PendingPanel method={method} formatCurrency={formatCurrency} amount={product.preco_promocional}
                checkoutUrl={checkoutUrl} reference={reference} entity={entity} />
            ) : (
              <>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="form-group">
                    <label className="form-label" htmlFor="telefone">
                      Telefone / WhatsApp {method.needsPhone ? '' : '(opcional)'}
                    </label>
                    <input type="tel" id="telefone" name="telefone" className="input-field" placeholder="9XX XXX XXX"
                      required={method.needsPhone} value={formData.telefone} onChange={handleInputChange} />
                  ...
                  <div className="form-group">
                    <p className="text-sm text-secondary mb-4">
                      * Pagamento processado de forma segura. Assim que for confirmado, os teus dados de acesso são entregues automaticamente por e-mail{formData.telefone ? ' e WhatsApp' : ''}.
                    </p>
                    <button type="submit" className="btn-primary w-full text-lg py-3" disabled={phase === 'submitting'}>
...
function PendingPanel({ method, formatCurrency, amount, checkoutUrl, reference, entity }) {
  return (
    <div className="text-center">
      ...
      <p className="text-secondary mb-6">Confirma o pagamento de <strong>{formatCurrency(amount)}</strong> para receberes o teu produto automaticamente.</p>

      <div className="glass p-4 rounded-lg mb-4" style={{ textAlign: 'left' }}>
        {method.value === 'multicaixa' && (
          <p className="text-sm">Vais receber um pedido de confirmação no teu telemóvel na app <strong>Multicaixa Express</strong>. Abre a app e aprova o pagamento.</p>
        )}
        ...
        {method.value === 'stripe' && (
          <div className="text-sm">
            ...
            {checkoutUrl
              ? <a href={checkoutUrl} target="_blank" rel="noopener noreferrer" className="btn-primary w-full"><ExternalLink size={18} /> Pagar com cartão</a>
              : <p className="text-secondary">A preparar a página de pagamento…</p>}
          </div>
        )}
      </div>
...
function DeliveredPanel({ product, credential, channels }) {
  ...
  const copy = () => {
    navigator.clipboard?.writeText(credential).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); });
  };
  return (
    <div className="text-center">
      <CheckCircle2 size={44} style={{ color: '#22c55e', margin: '0 auto 1rem' }} />
      <h2 className="text-2xl font-bold mb-2">Pagamento confirmado!</h2>
      <p className="text-secondary mb-6">A tua compra de <strong>{product.name}</strong> foi concluída. Aqui estão os teus dados de acesso:</p>
      ...
      <div className="glass p-4 rounded-lg mb-4" style={{ textAlign: 'left', position: 'relative' }}>
        ...
        {credential && (
          <button onClick={copy} className="btn-primary" style={{ position: 'absolute', top: '.5rem', right: '.5rem', padding: '.4rem .7rem', fontSize: '.8rem' }}>
      ...
      <p className="text-sm text-secondary">
        Enviámos também para {channels?.includes('email') ? 'o teu e-mail' : 'o teu e-mail'}{channels?.includes('whatsapp') ? ' e WhatsApp' : ''}.
      </p>
      <Link to="/" className="btn-primary mt-4">Voltar ao Catálogo</Link>
    </div>
  );
