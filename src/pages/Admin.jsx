...
export default function Admin() {
  ...
  useEffect(() => { if (token) load(token); /* auto-login se já houver token */ }, []); // eslint-disable-line
      ...
      <div className="container" style={{ maxWidth: 420, margin: '3rem auto' }}>
        <div className="checkout-form">
          ...
          <form onSubmit={(e) => { e.preventDefault(); load(token); }}>
            ...
            <button type="submit" className="btn-primary w-full text-lg py-3" disabled={loading || !token}>
    ...
    <div className="container animate-fade-in" style={{ paddingBottom: '3rem' }}>
      <div className="flex justify-between items-center mb-6" style={{ flexWrap: 'wrap', gap: 12 }}>
        ...
        <div className="flex gap-2">
          <button className="btn-primary" onClick={() => load(token)} style={ghost}><RefreshCw size={16} /> Atualizar</button>
          <button className="btn-primary" onClick={logout} style={ghost}><LogOut size={16} /> Sair</button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gap: '1.25rem' }}>
        {data.products.map((p) => (
          <ProductRow key={p.id} product={p} counts={data.stock_counts[p.id] || { available: 0, sold: 0 }} token={token} onSaved={() => load(token)} />
        ))}
      </div>
  ...
  return (
    <div className="glass p-4 rounded-lg">
      ...
      <div className="flex items-center gap-3">
        <button className="btn-primary" onClick={save} disabled={saving}>{saving ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Guardar banner</button>
...
function ProductRow({ product, counts, token, onSaved }) {
  ...
  const save = async () => {
    setSaving(true); setMsg('');
    try {
      await admin('update_product', { id: product.id, fields: {
        imagem: f.imagem, badge: f.badge,
        preco_promocional: Number(f.preco_promocional), preco_original: Number(f.preco_original),
        disponibilidade: f.disponibilidade, ativo: f.ativo,
      } }, token);
  ...
  return (
    <div className="glass p-4 rounded-lg">
      <div className="flex justify-between items-center mb-3" style={{ flexWrap: 'wrap', gap: 8 }}>
        ...
        <span className="text-sm" style={{ color: counts.available > 0 ? '#22c55e' : '#f87171' }}>
          Stock: {counts.available} disponíve{counts.available === 1 ? 'l' : 'is'} · {counts.sold} vendido{counts.sold === 1 ? '' : 's'}
        </span>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem' }}>
        <Field label="Imagem (URL)" value={f.imagem} onChange={(v) => setF({ ...f, imagem: v })} />
      ...
      <div className="flex items-center gap-3 mb-4" style={{ flexWrap: 'wrap' }}>
        ...
        <button className="btn-primary" onClick={gerarLink} style={{ ...ghost, padding: '.5rem 1rem' }}><Link2 size={15} /> Gerar link de checkout</button>
        {msg && <span className="text-sm text-secondary">{msg}</span>}
      </div>
      ...
      <div className="mt-3">
        ...
        <textarea className="input-field" rows={3} value={creds} onChange={(e) => setCreds(e.target.value)}
          placeholder={"email: conta@exemplo.com | senha: 1234\nemail: conta2@exemplo.com | senha: 5678"} style={{ resize: 'vertical', fontFamily: 'monospace', fontSize: '.85rem' }} />
        ...
        <table style={{ width: '100%', fontSize: '.85rem', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ textAlign: 'left', color: 'var(--text-secondary)' }}>
              <th style={th}>Data</th><th style={th}>Produto</th><th style={th}>Cliente</th><th style={th}>Valor</th><th style={th}>Método</th><th style={th}>Estado</th>
            </tr>
          </thead>
          <tbody>
            {orders.map((o) => (
...
function EstadoBadge({ status }) {
  const cores = {
    delivered: '#22c55e', paid: '#f59e0b', pending: '#94a3b8', failed: '#f87171', delivering: '#a78bfa',
  };
  ...
  return (
    <div className="mb-2">
      ...
      <div className="flex gap-2 items-center">
        ...
        <button className="btn-primary" onClick={copy} style={{ ...ghost, padding: '.5rem .7rem', whiteSpace: 'nowrap' }}><Copy size={14} /> {copied ? 'Copiado' : 'Copiar'}</button>
      </div>
    </div>
  );
}
...
const ghost = { background: 'transparent', border: '1px solid var(--border-color)' };
